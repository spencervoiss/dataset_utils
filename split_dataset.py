...
parser = ArgumentParser(description="Split your dataset into train, validation, and test sets")

parser.add_argument(
    "--input_dir", type=str, required=True, help="Directory containing the files to be split up"
)
parser.add_argument(
    "--img_dir",
    type=str,
...
parser.add_argument(
    "--output_dir", type=str, required=True, help="Directory for the split up dataset"
)
parser.add_argument(
    "-x",
    "--extension",
...
try:
    assert args.loglevel in [
        "cricital",
        "error",
        "warning",
        "info",
        "debug",
    ], "Logging level must be debug, info, warning, error, or critical. Setting logging level to INFO..."
...
if img_dir:
    ...
    os.makedirs(valid_imgs, exist_ok=True)
